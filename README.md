...
后台会从 OpenAlex、Crossref 与 arXiv 检索近期相关论文，按标题、摘要、期刊和关键词筛选二维半导体逻辑器件相关条目。若找到 arXiv HTML、开放 HTML、DOI 落地页或 Unpaywall/OpenAlex 给出的开放全文入口，系统会读取多个可访问来源并合并正文，从全文中抽取 `Ion`、`Rc`、`SS`、`VDS`、开关比等参数，同时保存每个字段对应的原文证据片段；若参数不完整，则进入“待补参数”候选库。
