...
后台会从 OpenAlex、Crossref 与 arXiv 检索近期相关论文，按标题、摘要、期刊和关键词筛选二维半导体逻辑器件相关条目。若找到 arXiv HTML、开放 HTML 或 Unpaywall/OpenAlex 给出的开放全文入口，系统会读取开放全文并从正文中抽取 `Ion`、`Rc`、`SS`、`VDS`、开关比等参数；若参数不完整，则进入“待补参数”候选库。

检索日志会显示完整过程：原始抓取条数、重复跳过条数、相关命中条数、开放全文读取篇数、全文补充参数篇数、入库/更新条数、可直接计算条数和待补参数条数。需要注意的是，Nature、IEEE 等数据库经常只开放标题和摘要，不开放完整器件表格；这种情况下网站会先把论文放入候选库，而不会编造参数。
