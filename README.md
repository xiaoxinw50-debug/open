...
若全文页面必须通过你的学校账号或出版商账号访问，可使用同一页面提供的“导入到 Γ2D”书签按钮：把它拖到浏览器书签栏，在已登录的论文全文页点击书签，当前页面可见文本会被发送回本网站的手动导入框并自动抽取。若你先选中一段图注、表格或参数段，书签会优先导入选中文字；未选中时才导入整页可见文本。该方式不保存账号、密码或 cookie，也不让 Render 服务器代替你登录；它只处理你当前浏览器已经合法显示的文本。
