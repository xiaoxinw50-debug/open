...
数值来源分三类显示和导出：`原文/核对值` 表示来自论文正文、图表、补充材料或人工复核；`派生值` 表示由其他原文值严格推出，例如由 `Ion` 和 `Ioff` 计算 `log10(Ion/Ioff)`；`估算值` 表示系统默认假设，只进入估算排序，不进入严格 Γ2D 计算。
...
待审核论文页集中显示估算样本和缺参样本。页面顶部提供数据诊断面板，统计严格计算、估算排序、待补参数和隐藏候选数量，并按缺失字段给出瓶颈排行。若需要排查检索噪声，可打开“显示低价值自动候选”开关，临时查看被过滤的综述、传感、光电或缺少 `Ion/Rc` 的自动候选。

待审核队列支持按“全部待审核”“缺 Rc 口径”“缺 Ion/Rc”“不能计算”筛选。默认把只差 `Rc` 口径的样本排在前面，因为这类样本通常只需要人工确认接触电阻定义，就可能从估算排序进入严格 Γ2D 排序。

当候选论文已经有 `Rc` 数值但缺少 `Rc` 口径时，表格操作列会出现“确认 Rc”按钮，可快速标记为“源漏总等效”或“单侧接触”。确认后系统会重新计算该论文；若其余字段齐全，样本会自动进入严格 Γ2D 排序。

对于缺少 `Rc` 口径但已经具备 `Ion/Rc` 的样本，待审核表会同时比较“源漏总等效”和“单侧接触”两种解释下的接触压降、有效电压余量和 Γ2D。若 VDS、SS 或开关比缺失，页面会把严格场景和按默认假设估算的场景分开显示；估算敏感性只用于辅助人工核对，不能替代原文中的接触电阻定义。
...
如果论文全文只能在浏览器、学校数据库或本地 PDF 中打开，推荐使用“手动计算”页的“全文不可自动获取时”文本导入框：从 PDF、网页、补充材料或图表说明中复制与器件参数相关的段落、图注和表格文字，点击“从文本抽取并回填参数”。系统会从这段人工导入文本中抽取参数、保留证据片段和来源标记，再按同一套 Γ2D 规则计算。这个流程比让服务器强行抓取付费全文更可靠，也更符合版权和访问权限边界。

若全文页面必须通过你的学校账号或出版商账号访问，可使用同一页面提供的“打开原文/登录”和“导入到 Γ2D”书签按钮：先打开原文页完成登录，再把“导入到 Γ2D”拖到浏览器书签栏，在已登录的论文全文页点击书签，当前页面可见文本会被发送回本网站的手动导入框并自动抽取。若你先选中一段图注、表格或参数段，书签会优先导入选中文字；未选中时才导入整页可见文本。该方式不保存账号、密码或 cookie，也不让 Render 服务器代替你登录；它只处理你当前浏览器已经合法显示的文本。

“自动检索”页还提供“账号登录辅助搜索”：输入关键词后可以直接跳转 Nature、IEEE Xplore 或知网的检索页完成登录，也可以一键同时打开 Nature/IEEE/知网检索页并启动网站自己的开放全文检索。登录页面中的全文仍通过“导入到 Γ2D”书签回传文本，不会把账号凭据交给服务器。

为了减少手工对应论文，排序表和待审核表中的每条论文还提供“登录导入”按钮。点击后，网站会在本机浏览器中记住这篇待处理论文并打开原文页；你登录并打开全文后点击“导入到 Γ2D”书签，网站会自动把导入文本匹配回这篇论文、抽取参数、保存记录并刷新排序。整个过程仍不上传账号凭据，只在你的浏览器本地保存一个待处理论文 ID。

如果浏览器不方便拖拽书签按钮，可点击“复制书签脚本”，新建一个浏览器书签并把网址粘贴为复制出的 `javascript:` 脚本。待导入状态条还提供“跳过并处理下一篇”，用于连续处理多篇需要登录全文的候选论文。

若关键数值只出现在扫描版表格、图注截图或论文图片中，手动计算页提供两个补数工具：

- `OCR 辅助导入`：上传裁剪后的参数表格、图注或正文截图，服务器用 Tesseract 识别中英文文字，并把识别文本追加到全文导入框，再走同一套参数抽取和证据记录。建议只上传参数附近截图，不要上传整页 PDF 截图。
- `半自动图表解析`：上传曲线图或柱状图截图，先输入坐标范围，再点击左下轴点、右上轴点和目标数据点。网站输出 `point,x,y,pixelX,pixelY` CSV。这个工具只做可复核的坐标数字化，不会自动猜测图中哪个点代表 `Ion`、`SS` 或 `Rc`；最终仍需要人工把取点目的、原图来源和坐标范围写入数据溯源。
...
4. 结果输出：`排序总览` 页面可复制完整 JSON，也可下载 CSV。输出字段包括论文信息、原始参数、数值来源、接触压降、有效电压余量、开关代价、Γ2D、判读和缺失字段。
...
- 接入更稳定的开放 PDF 解析、批量 OCR 和更完整的图表数字化审计记录。
- 增加 WPS/Word 附表导出。
- 增加人工审核队列与多人协作权限。
